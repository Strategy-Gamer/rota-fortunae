...
class FinalSim:
    def __init__(
        ...
        self.cull_env = 0.0           # low-passed envelope of the burst PULSE (spike_component) -> a wide "a wave is
                                      #   happening" signal that drives elite culling. It is NOT severity-tapered (the
                                      #   pulse saturates ~1 each burst), so late waves keep culling at full rate and
                                      #   E decays over a BOUNDED number of waves. The DISPLAY violence U_e tapers with
                                      #   severity for the graph/mortality/legitimacy; the cull envelope does not.
        self.cull_env_floor = 0.10    # envelope below this is the between-wave lull (no purge); above it a father-son
                                      #   wave is clearing a cohort of surplus elites. Set above the slow-decaying
                                      #   between-wave tail so the lulls hold E steady (a SEQUENCE of waves clears it).
        self.E_span = 2.0             # elite-surplus ratio (elites/positions - 1) that reads as a full-blown crisis
                                      #   (E=1). Peak overproduction runs ~1.6-2.0x, so peak E lands ~0.8. Larger =>
                                      #   E is a gentler read-out of the same overproduction (shrinks E's whole range).
        self.E_exit_thresh = 0.12     # fracture ends once overproduction is MOSTLY cleared (E=0.12 => ~24% residual
                                      #   surplus at E_span=2). The crisis need not grind the surplus to zero -- E
                                      #   keeps falling to 0 through early prosperity as elites slide back down. Set
        ...
        self.rad_wane = 0.09          # immunity waning rate (moderate -> naive); ~1/this sets the father-son
                                      #   period. Tuned so waves are ~40-45 ticks apart (a ~50yr father-son cycle:
                                      #   ~20 ticks of heightened violence + ~25 tick lull), per Turchin/Plantagenat.
        ...
        self.u_decay = 0.14           # U_e low-pass rate when violence is falling (slow decay -> wider ~15-20 tick
                                      #   "heightened instability" periods, matching the historical violence clusters)
        self.spike_gain = 1.4         # converts the excitable burst R (saturates ~0.7) into a 0..1 spike pulse, so a
                                      #   full father-son burst under full severity reaches ~1.0 (clamped).
        self.sev_exp = 0.55           # violence amplitude scales with overproduction as E**sev_exp. <1 makes the
                                      #   scaling CONCAVE: the taper across a fracture is visible (later waves smaller)
                                      #   but late waves stay strong enough to keep CULLING at a good rate, so the
        ...
        #      heals in the lulls. Legitimacy is a catch-all for the Mandate of Heaven (popular legitimacy)
        #      AND elite cooperation (rival/overproduced elites withdraw it). ----
        self.legitimacy = 1.0         # [0,1] slow stock; heals toward 1, eroded by unrest & elite fragmentation.
                                      #   Event-adjustable later (self.legitimacy += delta from game events).
        self.legit_recover = 0.03     # heal rate toward full legitimacy in calm (slow => ~a generation)
        self.legit_unrest = 0.35      # legitimacy lost per unit unrest ABOVE the floor (violence spikes crash it)
        self.legit_unrest_floor = 0.09 # unrest above this erodes legitimacy. Set BELOW the fracture hum (~0.13)
                                      #   but ABOVE the prosperity hum (~0.07): so the sustained crisis hum keeps
                                      #   legitimacy (and S) in a LOW BAND through fracture -- sustaining the
                                      #   father-son waves -- while spikes dip it further (ebb/flow) and calm heals it
        self.legit_frag = 0.05        # legitimacy lost per unit elite overproduction above the tolerance
        self.legit_frag_ok = 0.30     # overproduction (E) the elite can still cohere below (no cooperation loss)
        self.w_leg = 0.65             # weight of legitimacy in the S target (the dominant driver)
        self.w_fisc = 0.35            # weight of fiscal health in the S target (solvency still matters)
        self.health_adjust = 0.06     # S heal rate toward target (small => smooth, non-sawtooth recovery)
        self.health_adjust_down = 0.5 # S collapse rate (asymmetric: sharp drop when the target falls)

        # Rolling carrying-capacity memory for the population-pressure readout (see step()).
        self.cc_window = 100          # ticks of carrying-cap history the pressure gauge averages over
        self.cc_history = []
    ...
    def step(self, t, randomness=0.0):
        ...
        # doesn't instantly spike pressure -- the memory of recent capacity cushions it. (Inert while cap is
        # constant at 1.0; wired for when districts/tech make it dynamic.) Births/deaths still use the real
        # instantaneous cap -- only the pressure GAUGE (which drives wages/mobilization) is smoothed.
        self.cc_history.append(carrying_cap)
        if len(self.cc_history) > self.cc_window:
            self.cc_history.pop(0)
        cc_eff = max(carrying_cap, sum(self.cc_history) / len(self.cc_history))
        ...
        # ---- Legitimacy (Mandate of Heaven + elite cooperation): a slow [0,1] stock that heals toward full
        # in calm and erodes from active unrest (popular illegitimacy) and from elite overproduction above a
        # tolerance (rival/factional elites withdrawing cooperation). U_e/E are last-tick values (the same
        # one-tick lag this block already runs on). Event-adjustable later (self.legitimacy += event delta).
        legit_erosion = (self.legit_unrest * max(0.0, self.U_e - self.legit_unrest_floor)
                         + self.legit_frag * max(0.0, self.E - self.legit_frag_ok))
        self.legitimacy += self.legit_recover * (1.0 - self.legitimacy) - legit_erosion
        self.legitimacy = max(0.0, min(1.0, self.legitimacy))

        # State capacity S is now its own slow-drifting variable (no fracture pin): it integrates toward a
        # blend of LEGITIMACY (dominant) and fiscal health. It ebbs & flows because legitimacy dips with each
        # violence spike and heals in the lulls; sustained overproduction holds it down through fracture, and
        # it recovers as the crisis resolves. Asymmetric: sharp collapse when the target drops, slow heal up.
        # Because E reaches S only through the slow legitimacy stock, S doesn't instantly mirror 1-E.
        S_target = self.w_leg * self.legitimacy + self.w_fisc * fiscal_signal
        if S_target < self.S:
            self.S += (S_target - self.S) * self.health_adjust_down * self.dS_nmult   # fast collapse
        else:
            self.S += (S_target - self.S) * self.health_adjust * self.dS_mult         # slow heal
        ...
        if self.phase == 2:
            # During the crisis no one CLIMBS into a collapsing elite, but downward mobility resumes: as the
            # population crashes and commoner wages recover, overproduced elites slide back down to commoner
            # status. Throttled (frac_mobility_scale) so the WAVES do most of the clearing and E stays high
        ...
        # is a purge/civil war that clears a roughly CONSISTENT fraction of the surplus elite cohort -- so a
        # SEQUENCE of ~3-4 waves consumes the overproduction over a bounded-length fracture. Driven by the
        # (non-tapered) burst ENVELOPE rather than the severity-tapered display violence U_e, so late waves
        # keep culling at a good rate -- absolute deaths still shrink because elite_count itself is falling.
        ...
        # Observed violence = an endemic hum + the excitable spike, both scaled by SEVERITY = elite
        # overproduction E, so the AMPLITUDE tracks the SCALE OF THE PROBLEM: the worst overproduction gives
        # the biggest flare-ups and the waves shrink as the crisis clears its surplus elites. (E, not
        # `conditions`, sets the size -- `conditions` saturates to ~1 in every crisis by design, so it can't
        # carry magnitude; it still gates WHEN waves ignite. E is a clean unsaturated [0,1] magnitude now that
        # it reads real overproduction.) The hum is SUPPRESSED outside fracture (hum_quiet) so strain stays
        # visibly calm: unrest ACCUMULATES quietly (R charging toward ignition) then SPIKES at the
        # strain->fracture onset, instead of the baseline ramping up through strain. The ignition spike itself
        # is NOT gated, so that first burst still fires at full size and marks the transition.
        severity = self.E ** self.sev_exp                        # magnitude of the crisis (overproduction), concave
        spike_component = min(1.0, R * self.spike_gain)          # 0..1 excitable burst pulse
        # Cull envelope: low-pass the (severity-independent) burst pulse with the same fast-rise/slow-decay as
        # U_e, giving a WIDE "a wave is happening" window for elite culling that does NOT shrink as the crisis
        ...
        # (rel<=1: no surplus elite class to compete over the pie) and climbs ~linearly with the surplus, so
        # E reads the REAL overproduction fraction rather than sitting on a sigmoid floor (~0.14 at rel=1).
        # Linear (not sigmoid) so severity reads proportionally: twice the surplus -> ~twice the E -> ~twice
        # the EMP/violence pressure, i.e. "how much trouble it causes scales with the scale of the problem".
        self.overprod_ratio = rel                              # exposed for introspection/tuning
        overprod = max(0.0, rel - 1.0)                         # fractional surplus (0 = none, 1.0 = double the positions)
        self.E = max(0.0, min(1.0, overprod / self.E_span))    # E_span = surplus ratio that reads as full crisis (E=1)
        ...
        if self.phase == 0 and (self.E > 0.3 or self.P > 0.7):
            ...
            # into early prosperity (and, at low rad_suppress, so chronic radical noise can't keep culling
            # elites and prevent the next expansion from building).
            self.rad_N, self.rad_R, self.rad_M = 0.2, 0.0, 0.8
